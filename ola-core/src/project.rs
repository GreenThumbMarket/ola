...
impl ProjectManager {
    pub fn new() -> Result<Self> {
        let home =
            std::env::var("HOME").map_err(|_| anyhow::anyhow!("HOME directory not found"))?;
    ...
    pub fn load_project(&self, project_id: &str) -> Result<Option<Project>> {
        ...
        let project: Project =
            serde_json::from_str(&content).with_context(|| "Failed to parse project JSON")?;
    ...
    pub fn save_project(&self, project: &Project) -> Result<()> {
        ...
        let content =
            serde_json::to_string_pretty(project).with_context(|| "Failed to serialize project")?;
    ...
    pub fn set_active_project(&self, project_id: &str) -> Result<()> {
        ...
        fs::write(&active_file, project_id).with_context(|| {
            format!(
                "Failed to write active project file: {}",
                active_file.display()
            )
        })?;
    ...
    pub fn get_active_project(&self) -> Result<Option<String>> {
        ...
        let content = fs::read_to_string(&active_file).with_context(|| {
            format!(
                "Failed to read active project file: {}",
                active_file.display()
            )
        })?;
